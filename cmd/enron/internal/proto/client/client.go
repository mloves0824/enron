...
var CmdClient = &cobra.Command{
	...
	Long:  "Generate the proto client code. Example: enron proto client helloworld.proto",
	Run:   run,
}
...
func init() {
	if protoPath = os.Getenv("ENRON_PROTO_PATH"); protoPath == "" {
...
func run(_ *cobra.Command, args []string) {
	...
	if err = look("protoc-gen-go", "protoc-gen-go-grpc", "protoc-gen-go-http", "protoc-gen-go-errors", "protoc-gen-openapi"); err != nil {
		// update the enron plugins
		cmd := exec.Command("enron", "upgrade")
...
func generate(proto string, args []string) error {
	...
	inputExt := []string{
		"--proto_path=" + base.EnronMod(),
		"--proto_path=" + filepath.Join(base.EnronMod(), "third_party"),
