...
all:
	@cd cmd/enron && go build && cd - &> /dev/null
...
#!root, install for current user
	$(shell if [ -z '$(INSTALL_BIN)' ]; then read -p "Please select installdir: " REPLY; mkdir -p $${REPLY};\
	cp ./cmd/enron/enron $${REPLY}/;cp ./cmd/protoc-gen-go-errors/protoc-gen-go-errors $${REPLY}/;cp ./cmd/protoc-gen-go-http/protoc-gen-go-http $${REPLY}/;else mkdir -p '$(INSTALL_BIN)';\
	cp ./cmd/enron/enron '$(INSTALL_BIN)';cp ./cmd/protoc-gen-go-errors/protoc-gen-go-errors '$(INSTALL_BIN)';cp ./cmd/protoc-gen-go-http/protoc-gen-go-http '$(INSTALL_BIN)'; fi)
endif
...
.PHONY: uninstall
uninstall:
	$(shell for i in `which -a enron | grep -v '/usr/bin/enron' 2>/dev/null | sort | uniq`; do read -p "Press to remove $${i} (y/n): " REPLY; if [ $${REPLY} = "y" ]; then rm -f $${i}; fi; done)
